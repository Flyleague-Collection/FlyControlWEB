...
const getAnnouncements = async (page: number, pageSize: number): Promise<PageListResponse<AnnouncementModel>> => {
    ...
    const data = await Announcement.getDetailAnnouncements(page, pageSize);
    if (data != null) {
...
const publishAnnouncement = async () => {
    ...
    if (await Announcement.createAnnouncement(announcementFormData.value)) {
        showSuccess("发布公告成功");
...
const editAnnouncement = async () => {
    ...
    if (await Announcement.updateAnnouncement(oldData.value?.id, updateData)) {
        showSuccess("修改公告成功");
        announcementsRef.value?.flushData();
...
const deleteAnnouncement = async () => {
    ...
    if (await Announcement.deleteAnnouncement(announcementId.value)) {
        showSuccess("删除计划成功");
        announcementsRef.value?.flushData();
    }
}
...
<template>
            ...
            <template #default="scope">
                <el-descriptions :column="1" border label-width="80">
                    ...
                    <el-descriptions-item label="内容">
                        <div v-html="DOMPurify.sanitize(marked.parse(scope.row.content))"/>
                    </el-descriptions-item>
                </el-descriptions>
            </template>
        </el-table-column>
    ...
    <FormDrawer ref="publishAnnouncementDrawerRef" title="发布公告" @drawer-confirm-event="publishAnnouncement()"
                @drawer-open-event="getPublishAnnouncementRef()">
    ...
    <FormDrawer ref="editAnnouncementDrawerRef" title="编辑公告" @drawer-confirm-event="editAnnouncement()"
                @drawer-open-event="getEditAnnouncementRef()">
