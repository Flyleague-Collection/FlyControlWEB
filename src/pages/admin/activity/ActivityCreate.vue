...
// @ts-ignore
const activityForm: Ref<FormInstance> = ref<FormInstance>()
const createNewActivity = async () => {
    if (!await activityForm.value?.validate()) {
        return
    }

    const response = await request.post(`/activities`, activity.value)
...
<template>
    <el-card footer-class="flex justify-item-flex-end">
        ...
        <el-card>
            ...
            <el-form :model="activity" label-position="right" label-width="auto" ref="activityForm">
                <el-form-item label="活动名">
                    <el-input v-model="activity.title"/>
                </el-form-item>
                <el-form-item label="活动时间">
                    <el-date-picker type="datetime" v-model="activity.active_time"
        ...
        <template #footer>
            <el-button :icon="Check" type="success" @click="createNewActivity">保存</el-button>
            <el-button :icon="RefreshLeft" type="warning" @click="resetConfirmDialog = true">重置</el-button>
            <el-button :icon="Close" type="danger" @click="cancelConfirmDialog = true">取消</el-button>
        </template>
    </el-card>
    <ConfirmDialog v-model="resetConfirmDialog" header-content="重置已输入的内容"
                   body-content="所有未保存的内容将会丢失, 确认继续吗？"/>
    <ConfirmDialog v-model="cancelConfirmDialog" header-content="取消创建"
                   body-content="所有未保存的内容将会丢失, 确认继续吗？"/>
</template>
